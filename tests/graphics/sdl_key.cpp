...
namespace sdl {
    ...
    // make keys printable
    static void PrintTo(const Key& key, std::ostream* os) { //NOLINT(misc-use-internal-linkage)
        *os << key.to_string();
    }


    static std::ostream& operator<<(std::ostream& os, const Key& value) { //NOLINT(misc-use-internal-linkage)
        os << value.to_string();
        return os;
    }
} // namespace sdl
