...
NAN_METHOD(getInformation) {
  ...
  if (not std::filesystem::exists(filePath)) {
    ...
    info.GetIsolate()->ThrowException(
        Nan::Error(Nan::New(error).ToLocalChecked()));
    return;
  }
  ...
  auto json_value =
      json::try_convert_to_json<recorder::RecordingReader>(recording_reader);
  ...
  v8::Local<v8::String> js_json_string =
      Nan::New(result_string_json).ToLocalChecked();
  ...
  info.GetIsolate()->ThrowException(
      Nan::Error("Failed to parse internal JSON structure!"));
  return;
}

NAN_MODULE_INIT(InitAll) {
  Nan::Set(target, Nan::New("isRecordingFile").ToLocalChecked(),
           Nan::GetFunction(Nan::New<v8::FunctionTemplate>(isRecordingFile))
               .ToLocalChecked());

  Nan::Set(target, Nan::New("getInformation").ToLocalChecked(),
           Nan::GetFunction(Nan::New<v8::FunctionTemplate>(getInformation))
               .ToLocalChecked());

  Nan::Set(target, Nan::New("version").ToLocalChecked(),
           Nan::New<v8::String>(utils::version()).ToLocalChecked());

  v8::Local<v8::Object> properties = Nan::New<v8::Object>();

  std::vector<std::pair<std::string, u32>> properties_vector{
      {"height", grid::height_in_tiles}, {"width", grid::width_in_tiles}};
  ...
  for (const auto &[key, value] : properties_vector) {
    ...
    grid_properties
        ->Set(Nan::GetCurrentContext(), keyValue, Nan::New<v8::Uint32>(value))
        .Check();
  }

  properties
      ->Set(Nan::GetCurrentContext(),
            Nan::New<v8::String>("gridProperties").ToLocalChecked(),
            grid_properties)
      .Check();
