...
namespace utils {
    ...
    [[nodiscard]] std::filesystem::path get_assets_folder() {
        ...
        return std::filesystem::path{ "" };
#elif defined(__SWITCH__) or defined(__3DS__)
        // this is in the internal storage of the nintendo switch  /3ds, it is mounted by libnx (runtime switch support library) or libctru (runtime 3ds support library) and filled at compile time with assets (its called ROMFS there)
