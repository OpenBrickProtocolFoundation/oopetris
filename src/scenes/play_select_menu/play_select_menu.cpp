...
namespace scenes {
    ...
    PlaySelectMenu::PlaySelectMenu(ServiceProvider* service_provider, const  ui::Layout& layout)
        ...
        const auto multi_player_button_id = id_helper.index();
        m_main_grid.add<ui::Button>(
                multi_player_button_id, service_provider, "Multi Player",
                service_provider->fonts().get(FontId::Default), Color::white(), id_helper.focus_id(),
                [this](const ui::Button&) { m_next_command = Command::MultiPlayer; }, button_size, button_alignment,
                button_margins
        );
        m_main_grid.get<ui::Button>(multi_player_button_id)->disable();
