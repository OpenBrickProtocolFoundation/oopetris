...
bool GameManager::is_active_tetromino_position_valid() {
    if (!m_active_tetromino) {
        return true;
    }
    for (const Mino& mino : m_active_tetromino->minos()) {
        if (mino.position().x < 0 || mino.position().x >= Grid::width || mino.position().y >= Grid::height
            || !m_grid.is_empty(mino.position())) {
            return false;
        }
    }
    return true;
