...
[[nodiscard]] tl::expected<bool, std::string> Connection::is_data_available(Uint32 timeout_ms) {
    ...
    const auto num_sockets = SDLNet_TCP_AddSocket(socket_set, m_socket);
    if (num_sockets != 1) {
        SDLNet_FreeSocketSet(socket_set);
        return tl::make_unexpected("SDLNet_AddSocket failed, this is an implementation error");
    }

    const auto result = SDLNet_CheckSockets(socket_set, timeout_ms);
    if (result == -1) {
        SDLNet_FreeSocketSet(socket_set);
...
[[nodiscard]] tl::expected<RawBytes, std::string> Connection::get_all_data_blocking() {
    ...
    while (true) {
        const int received_length = SDLNet_TCP_Recv(m_socket, memory, Connection::chunk_size);
        if (received_length <= 0) {
            free(memory);
            return tl::make_unexpected("SDLNet_TCP_Recv: " + std::string{ SDLNet_GetError() });
        }

        if (received_length != Connection::chunk_size) {

            return RawBytes{ static_cast<uint8_t*>(memory), data_size + received_length };
        }
...
[[nodiscard]] tl::expected<std::vector<RawTransportData>, std::string> Connection::get_data() {

    const auto data_available = is_data_available();
    if (!data_available.has_value()) {
        return tl::make_unexpected("in is_data_available: " + data_available.error());
    }

    if (!data_available.value()) {
        return std::vector<RawTransportData>{};
    }

    const auto data = get_all_data_blocking();
    if (!data.has_value()) {
        return tl::make_unexpected("in get_all_data_blocking: " + data.error());
    }


    const RawBytes raw_bytes = data.value();

    const auto result = RawTransportData::from_raw_bytes(raw_bytes);
    if (!result.has_value()) {
...
Connection::wait_for_data(std::size_t abort_after, Uint32 ms_delay) {
    ...
    while (true) {
        ...
        const auto data_available = is_data_available();
        if (!data_available.has_value()) {
            return tl::make_unexpected("In Connection::wait_for_data: " + data_available.error());
        }
        ...
        if (data_available.value()) {
            ...
            const auto data_vector = result.value();
            if (data_vector.size() == 0) {
                return tl::make_unexpected(
                        "In Connection::wait_for_data: waited for data, but now no data is available?? This is an "
                        "implementation bug!"
                );
            }
        ...
        const auto elapsed_time = SDL_GetTicks64() - start_time;
...
[[nodiscard]] tl::optional<std::shared_ptr<Connection>> Server::try_get_client() {
    ...
    const auto connection = std::make_shared<Connection>(client);
    m_connections.push_back(connection);
    return connection;
}

[[nodiscard]] tl::optional<std::shared_ptr<Connection>> Server::get_client(Uint32 ms_delay, std::size_t abort_after) {
    const auto start_time = SDL_GetTicks64();
    while (true) {

        const auto client = try_get_client();
        if (client.has_value()) {
            return client;
        }
