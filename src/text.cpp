...
Text::Text(Point position, Color color, std::string text, std::shared_ptr<Font> font)
    : m_position{ position },
      m_color{ color },
      m_text{ text },
      m_font{ font } { }
...
void Text::render(const Application& app) const {
    ...
    SDL_Surface* surface = TTF_RenderText_Solid(m_font->m_font, m_text.c_str(), textColor);
