...
Grid::Grid(Point offset, usize tile_size) : m_offset{ offset - Point{ 0, static_cast<SDL_int>(invisible_rows * tile_size) }}, m_tile_size{ tile_size } {
}

[[nodiscard]] Point Grid::tile_size() const {
    return Point{ static_cast<SDL_int>(m_tile_size), static_cast<SDL_int>(m_tile_size) };
}

[[nodiscard]] Point Grid::to_screen_coords(Point grid_coords) const {
    return m_offset + grid_coords * static_cast<SDL_int>(m_tile_size);
}
...
void Grid::draw_background(const ServiceProvider& service_provider, Rect grid_rect) const {
    const auto top_left = m_offset + Point{ 0, static_cast<SDL_int>(m_tile_size * invisible_rows) }
                          + grid_rect.top_left * static_cast<SDL_int>(m_tile_size);
    const auto bottom_right =
            top_left + Point{ grid_rect.width(), grid_rect.height() } * static_cast<SDL_int>(m_tile_size);
    ...
    for (usize column = 0; column <= static_cast<usize>(grid_rect.width()); ++column) {
        const auto start = top_left + Point{ static_cast<SDL_int>(column * m_tile_size), 0 };
        const auto end = Point{ start.x, static_cast<SDL_int>(start.y + grid_rect.height() * m_tile_size) };
    ...
    for (usize row = 0; row <= static_cast<usize>(grid_rect.height()); ++row) {
        const auto start = top_left + Point{ 0, static_cast<SDL_int>(row * m_tile_size) };
    ...
    const auto outline_bottom_right = Point{
        static_cast<SDL_int>(top_left.x + grid_rect.width() * m_tile_size + 1),
        static_cast<SDL_int>(top_left.y + grid_rect.height() * m_tile_size + 1),
    };
